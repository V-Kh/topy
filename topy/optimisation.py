...
def optimise(topology, save=True, dir='./iterations'):
    ...
    def _optimise(t):
        ...
        format_ = (t.itercount, t.objfval, t.desvars.mean(), t.change, t.p, t.q, t.eta.mean(), t.svtfrac)
    ...
    format_ = ('Iter', 'Obj. func.  ', 'Vol. ', 'Change    ', 'P_FAC', 'Q_FAC', 'Ave ETA', 'S-V frac.')
    logger.info(str_ % format_)
    logger.info('-' * 80)
    ti = time()
    ...
    logger.info('\nSolid plus void to total elements fraction = %3.5f' % (topology.svtfrac))
    # Print iteration info:

    logger.info('%d iterations took %3.3f minutes (%3.3f seconds/iteration)' % (topology.itercount, (te - ti) / 60, (te - ti) / topology.itercount))
    logger.info('Average of all ETA\'s = %3.3f (average of all a\'s = %3.3f)' % (array(etas_avg).mean(), 1 / array(etas_avg).mean() - 1))
