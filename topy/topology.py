...
class Topology:
    ...
    def set_top_params(self):
        ...
        logger.info(
            'Element type (ELEM_K) = {}'.format(self.topydict['ELEM_TYPE']))
        ...
        if self.dofpn == 1:
            ...
            self.alldof = np.arange(
                self.dofpn * (self.nelx + 1) * (self.nely + 1))
            self.desvars = np.zeros((self.nely, self.nelx)) + self.volfrac
        else:
            self.alldof = np.arange(self.dofpn * (self.nelx + 1) * \
                                    (self.nely + 1) * (self.nelz + 1))
            self.desvars = np.zeros((self.nelz, self.nely, self.nelx)) + \
                           self.volfrac
        self.df = np.zeros_like(
            self.desvars)  # Derivatives of obj. func. (array)
        ...
        self.dfree = np.zeros_like(
            self.rfree)  # Modified load vector (free dof)
        ...
        if self.approx == 'dquad':
            logger.info(
                'Using diagonal quadratic approximation (APPROX = dquad)')
    ...
    def sens_analysis(self):
        ...
        if self.nelz == 0:  # 2D problem
            ...
            e2sdofmap = np.add(e2sdofmap,
                               self.dofpn * (Y + X * (self.nely + 1)))
            Qe = self.d[e2sdofmap]
            ...
            e2sdofmap = np.expand_dims(self.e2sdofmapi.reshape(-1, 1, 1),
                                       axis=1)
        ...
        while (lam2 - lam1) / (lam2 + lam1) > 1e-8 and lam2 > 1e-40:
            ...
            if self.probtype == 'mech':
                if self.approx == 'dquad':
                    ...
                    desvars = np.maximum(VOID, np.maximum((self.desvars - move), np.minimum(SOLID, np.minimum((self.desvars + move), (self.desvars * np.maximum(1e-10, (-self.df / lammid)) ** self.eta) ** self.q))))
                ...
                if self.approx == 'dquad':
                    ...
                    beta = np.maximum(self.desvars - (self.df + lammid) / curv,
                                      VOID)
                    move_upper = np.minimum(move, self.desvars / 3)
                    desvars = np.maximum(VOID, np.maximum((self.desvars - move), np.minimum(SOLID, np.minimum((self.desvars + move_upper), beta ** self.q))))
                else:  # reciprocal or exponential
                    desvars = np.maximum(VOID, np.maximum((self.desvars - move), np.minimum(SOLID, np.minimum((self.desvars + move), (self.desvars * (-self.df / lammid) ** self.eta) ** self.q))))
